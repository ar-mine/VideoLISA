...
from videolisa.model.sam2.modeling.sam2_base import NO_OBJ_SCORE, SAM2Base
from videolisa.model.sam2.utils.misc import concat_points, fill_holes_in_mask_scores, load_video_frames
...
class SAM2VideoPredictor(SAM2Base):
    ...
    def from_pretrained(cls, model_id: str, **kwargs) -> "SAM2VideoPredictor":
        ...
        """
        from videolisa.model.sam2.build_sam import build_sam2_video_predictor_hf
    ...
    def add_text_embeddings(
        ...
        """Add new texts to a frame."""
        obj_idx = self._obj_id_to_idx(inference_state, obj_id)
        point_inputs_per_frame = inference_state["point_inputs_per_obj"][obj_idx]
        mask_inputs_per_frame = inference_state["mask_inputs_per_obj"][obj_idx]
        text_inputs_per_frame = inference_state["text_inputs_per_obj"][obj_idx]

        # if (texts is not None) != (labels is not None):
        #     raise ValueError("texts and labels must be provided together")
        if texts is None:
        ...
        #     labels = torch.tensor(labels, dtype=torch.int32)
        if texts.dim() == 2:
            texts = texts.unsqueeze(0)  # add batch dimension
        # if labels.dim() == 1:
        #     labels = labels.unsqueeze(0)  # add batch dimension

        texts = texts.to(inference_state["device"])
        # labels = labels.to(inference_state["device"])

        point_inputs_per_frame.pop(frame_idx, None)
        mask_inputs_per_frame.pop(frame_idx, None)
        text_inputs_per_frame[frame_idx] = texts
        # If this frame hasn't been tracked before, we treat it as an initial conditioning
        # frame, meaning that the inputs points are to generate segments on this frame without
        # using any memory from other frames, like in SAM. Otherwise (if it has been tracked),
        # the input points will be used to correct the already tracked masks.
